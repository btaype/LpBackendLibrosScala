    ...
    case req @ POST -> Root / "comprar" =>
      for {
        ...
        resultado <- LibroQueries.comprar_libro(
          datos.id_libro, datos.id_persona, datos.numero, datos.ccv_numero
        )

        _ <- IO(println(s"[DEBUG] Resultado desde DB: $resultado"))

        _ <- if (resultado.startsWith("OK")) {
          LibroQueries.obtener_correos_pdf(datos.id_persona, datos.id_libro).flatMap {
            case Some(info) =>
              IO(println(s"[DEBUG] Enviando correo a ${info.correo} con PDF ${info.nombrePDF}")) *>
              Enviar_correo.enviar_libro_coreo(info.correo, info.nombrePDF)
            case None =>
              IO(println("[ERROR] No se pudo obtener el correo o el PDF desde la BD"))
          }
        } else IO.unit

        res <- {
          if (resultado.startsWith("OK")) Ok(Map("ok" -> resultado).asJson)
          else BadRequest(Map("error" -> resultado).asJson)
        }
      } yield res
  }
}
