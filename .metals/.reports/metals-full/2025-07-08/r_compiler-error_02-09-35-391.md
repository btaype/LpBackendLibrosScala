...
object Paginafillter {
  ...
  val rutas = HttpRoutes.of[IO] {
    case req @ GET -> Root / "pagina_filtro" =>
      val idCatOpt     = req.uri.query.params.get("id_categoria").flatMap(s => scala.util.Try(s.toInt).toOption)
      val precioMinOpt = req.uri.query.params.get("precio_min").flatMap(s => scala.util.Try(BigDecimal(s)).toOption)
      val precioMaxOpt = req.uri.query.params.get("precio_max").flatMap(s => scala.util.Try(BigDecimal(s)).toOption)
      val ordenOpt     = req.uri.query.params.get("orden").flatMap(s => scala.util.Try(s.toBoolean).toOption)

      (idCatOpt, precioMinOpt, precioMaxOpt, ordenOpt) match {
        case (Some(idCategoria), Some(precioMin), Some(precioMax), Some(orden)) =>
          for {
            resultado <- LibroQueries.paginafiltro(idCategoria, precioMin, precioMax, orden)
