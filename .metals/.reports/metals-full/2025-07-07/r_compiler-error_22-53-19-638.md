...
object Comprarlibros {
 ...
 val rutas = HttpRoutes.of[IO] {
  case req @ POST -> Root / "comprar_libro" =>
    for {
      datos <- req.as[Comprarlibross]

      resultado <- LibroQueries.comprar_libro(
        datos.idLibro, datos.id_ersona, datos.numTarjeta, datos.ccvtxt
      )

      _ <- if (resultado.startsWith("OK")) {
       
        LibroQueries.obtenerCorreoYpdf(datos.id_ersona, datos.idLibro).flatMap {
          case Some(info) =>
            CorreoService.enviarPDF(info.correo, info.nombrePDF)
          case None =>
            IO(println("No se pudo obtener correo o PDF"))
        }
      } else IO.unit
