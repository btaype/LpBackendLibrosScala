...
object Paginafillter {
  ...
  val rutas = HttpRoutes.of[IO] {
    case req @ GET -> Root / "obtener_librosfiltro" =>
      ...
      val precioMinOpt = req.uri.query.params.get("precio_minimo").flatMap(s => scala.util.Try(BigDecimal(s)).toOption)
      val precioMaxOpt = req.uri.query.params.get("precio_maximo").flatMap(s => scala.util.Try(BigDecimal(s)).toOption)
